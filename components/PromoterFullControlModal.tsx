...
import { 
    XIcon, UserIcon, WhatsAppIcon, CheckCircleIcon, 
    RefreshIcon, MapPinIcon, ShieldCheckIcon
} from './Icons';
    ...
    return (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-sm z-[150] flex items-center justify-center p-4 md:p-8" onClick={onClose}>
            <div className="bg-secondary w-full max-w-4xl max-h-[90vh] rounded-[3rem] border border-white/10 shadow-2xl flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}>
                ...
                <div className="flex bg-dark/20 p-2 gap-2 border-b border-white/5">
                    <button onClick={() => setActiveTab('tasks')} className={`px-6 py-2.5 text-[10px] font-black uppercase rounded-xl transition-all ${activeTab === 'tasks' ? 'bg-primary text-white shadow-lg' : 'text-gray-500 hover:bg-white/5'}`}>Histórico Tasks</button>
                    <button onClick={() => setActiveTab('data')} className={`px-6 py-2.5 text-[10px] font-black uppercase rounded-xl transition-all ${activeTab === 'data' ? 'bg-primary text-white shadow-lg' : 'text-gray-500 hover:bg-white/5'}`}>Dados de Perfil</button>
                    <button onClick={() => setActiveTab('personal')} className={`px-6 py-2.5 text-[10px] font-black uppercase rounded-xl transition-all ${activeTab === 'personal' ? 'bg-primary text-white shadow-lg' : 'text-gray-500 hover:bg-white/5'}`}>Dados Pessoais</button>
                </div>
                            ...
                            {isLoadingTasks ? <div className="flex justify-center py-10"><RefreshIcon className="w-8 h-8 animate-spin text-primary" /></div> : assignments.map(a => (
                                <div key={a.id} className="bg-dark/40 p-4 rounded-2xl border border-white/5 flex justify-between items-center">
                                    ...
                                    <div className="flex gap-2">
                                        {a.proofImageUrls && <button onClick={() => setPhotoViewer({ isOpen: true, urls: a.proofImageUrls!, index: 0 })} className="p-2 bg-gray-800 rounded-xl text-primary"><RefreshIcon className="w-4 h-4"/></button>}
                                    </div>
                                </div>
                            ))}
                    ...
                    {activeTab === 'data' && (
                        <form onSubmit={handleSave} className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-fadeIn">
                             <div className="space-y-4">
                                <div><label className="text-[9px] font-black text-gray-500 uppercase ml-1">Nome</label><input value={formData.name || ''} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full bg-dark border border-gray-700 rounded-xl p-3 text-white text-sm" /></div>
                             ...
                             <div className="space-y-4">
                                ...
                                <button type="submit" disabled={isSaving} className="w-full py-4 bg-primary text-white font-black rounded-xl uppercase text-[10px] tracking-widest">{isSaving ? 'SALVANDO...' : 'ATUALIZAR PERFIL'}</button>
                             </div>
                        </form>
                    )}
                        ...
                        <div className="space-y-8 animate-fadeIn">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="bg-dark/40 p-5 rounded-2xl border border-white/5">
                                    <p className="text-[9px] font-black text-primary uppercase tracking-widest mb-3 flex items-center gap-2"><ShieldCheckIcon className="w-4 h-4"/> Documentação</p>
                                    <div className="space-y-1">
                                        <p className="text-xs text-gray-500 font-bold uppercase">CPF / Tax ID:</p>
                                        <p className="text-lg font-black text-white font-mono">{promoter.taxId || 'Não Informado'}</p>
                                    </div>
                                    <div className="space-y-1 mt-4">
                                        <p className="text-xs text-gray-500 font-bold uppercase">Data de Nascimento:</p>
                                        <p className="text-lg font-black text-white">{promoter.dateOfBirth ? new Date(promoter.dateOfBirth).toLocaleDateString('pt-BR') : '---'}</p>
                                    </div>
                                </div>
                                <div className="bg-dark/40 p-5 rounded-2xl border border-white/5">
                                    <p className="text-[9px] font-black text-primary uppercase tracking-widest mb-3 flex items-center gap-2"><MapPinIcon className="w-4 h-4"/> Endereço</p>
                                    {promoter.address ? (
                                        <div className="space-y-2">
                                            <p className="text-white font-bold text-sm uppercase">{promoter.address.street}, {promoter.address.number}</p>
                                            <p className="text-gray-400 text-xs font-bold uppercase">{promoter.address.city}</p>
                                            <p className="text-gray-500 text-xs font-mono">{promoter.address.zipCode}</p>
                                        </div>
                                    ) : <p className="text-gray-600 text-xs font-bold italic uppercase py-4">Endereço não cadastrado</p>}
                                </div>
                            </div>
                            
                            <div className="p-4 bg-blue-900/10 border border-blue-500/20 rounded-2xl">
                                <p className="text-[10px] text-blue-300 font-bold uppercase text-center">Os dados acima são confidenciais e protegidos pela LGPD.</p>
                            </div>
                        </div>
                    )}
                </div>
