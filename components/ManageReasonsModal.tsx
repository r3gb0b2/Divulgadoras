    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-lg max-h-[90vh] flex flex-col">
        <div className="flex justify-between items-center mb-4">
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Gerenciar Motivos de Rejeição</h2>
            <button onClick={onClose} className="text-gray-400 hover:text-gray-600 text-3xl">&times;</button>
        </div>

        <div className="space-y-4 mb-4">
            <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-200">Adicionar Novo Motivo</h3>
            <div className="flex gap-2">
                <input
                    ...
                    className="flex-grow mt-1 w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-gray-200 focus:outline-none focus:ring-primary focus:border-primary"
                />
        ...
        <div className="flex-grow overflow-y-auto border-t border-b dark:border-gray-700 py-4">
            <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-2">Motivos Existentes</h3>
            {isLoading && <p>Carregando...</p>}
            {error && <p className="text-red-500">{error}</p>}
            <ul className="space-y-2">
                {reasons.map(reason => (
                    <li key={reason.id} className="flex items-center justify-between p-2 bg-gray-50 dark:bg-gray-700/50 rounded-md">
                        {editingReason?.id === reason.id ? (
                            <input
                                type="text"
                                value={editingReason.text}
                                onChange={(e) => setEditingReason({ ...editingReason, text: e.target.value })}
                                className="flex-grow px-2 py-1 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800"
                            />
                        ) : (
                            <p className="text-gray-800 dark:text-gray-200">{reason.text}</p>
                        )}
                        <div className="flex gap-2 ml-4">
                            {editingReason?.id === reason.id ? (
                                <>
                                    <button onClick={handleUpdateReason} className="text-green-600 hover:text-green-800">Salvar</button>
                                    <button onClick={() => setEditingReason(null)} className="text-gray-500 hover:text-gray-700">Cancelar</button>
                                </>
                            ) : (
                                <>
                                    <button onClick={() => setEditingReason(reason)} className="text-indigo-600 hover:text-indigo-800">Editar</button>
                                    <button onClick={() => handleDeleteReason(reason.id)} className="text-red-600 hover:text-red-800">Excluir</button>
                                </>
                            )}
                        </div>
                    </li>
