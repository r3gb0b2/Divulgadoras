...
exports.cleanupDuplicateReminders = functions.region("southamerica-east1").https.onCall(async (data, context) => {
    ...
    try {
        ...
        snapshot.docs.forEach(doc => {
            ...
            // Use server timestamp or fallback to 0
            ...
            if (assignmentMap.has(assignmentId)) {
                ...
                // Keep the one created MOST RECENTLY (highest timestamp)
                if (createdAt > existing.createdAt) {
                    // New one is newer, delete the old one
                    ...
                    // New one is older (or same), delete the new one
