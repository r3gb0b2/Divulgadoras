...
exports.notifyPostPush = functions.region("southamerica-east1").https.onCall(async (data, context) => {
    ...
    // Busca tokens FCM em chunks (limite do where in é 30)
                        ...
                        <div style="font-family: sans-serif; color: #333; max-width: 600px; margin: auto; border: 1px solid #eee; padding: 20px; border-radius: 15px;">
                            ...
                            <div style="text-align: center; margin-top: 30px;">
                                <a href="https://divulgadoras.vercel.app/#/status?email=${encodeURIComponent(oldData.email)}" 
                                   style="display: inline-block; padding: 15px 35px; background: #7e39d5; color: white; border-radius: 10px; text-decoration: none; font-weight: bold; font-size: 16px;">
                                   ACESSAR MEU PORTAL
                                </a>
                            </div>
                        </div>
