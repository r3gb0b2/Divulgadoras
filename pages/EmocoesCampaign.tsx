...
const EmocoesCampaign: React.FC = () => {
    ...
    const handleProceedToPayment = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // 1. Se não existir perfil, cria um perfil básico de divulgadora (marcado como VIP org)
            if (!pId) {
                pId = await createVipPromoter({ name, email, whatsapp: sanitizedWhatsapp });
            }
            
            // 2. Garante que o perfil tenha os contatos atualizados para o VIP
            ...
            // 3. Chama a Cloud Function para gerar o Pix no Mercado Pago
                ...
                <div className="p-8">
                    ...
                    {step === 'select_event' && (
                        <div className="grid gap-4">
                            <h2 className="text-lg font-black text-white uppercase tracking-widest text-center mb-2">Escolha seu Acesso</h2>
                            {events.map(ev => (
                                <button key={ev.id} onClick={() => { setSelectedEvent(ev); setStep('benefits'); }} className="bg-dark/40 p-6 rounded-3xl border border-white/5 hover:border-primary text-left transition-all group flex justify-between items-center">
                                    <div>
                                        <p className="text-white font-black text-lg uppercase group-hover:text-primary transition-colors">{ev.name}</p>
                                        <p className="text-[10px] text-gray-500 font-black uppercase mt-1">Acesso Imediato</p>
                                    </div>
                    ...
                    {step === 'confirm_data' && (
                        <form onSubmit={handleProceedToPayment} className="space-y-4">
                            ...
                            <div className="relative">
                                ...
                                <input type="text" required value={name} onChange={e => setName(e.target.value)} className="w-full p-5 pl-14 bg-dark border border-white/10 rounded-3xl text-white outline-none focus:ring-2 focus:ring-primary font-bold" placeholder="Nome Completo" />
                            </div>
                            
                            <div className="relative">
                                <PhoneIcon className="absolute left-5 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-500" />
                                <input type="tel" required value={whatsapp} onChange={e => setWhatsapp(e.target.value)} className="w-full p-5 pl-14 bg-dark border border-white/10 rounded-3xl text-white outline-none focus:ring-2 focus:ring-primary font-bold" placeholder="WhatsApp (DDD + Número)" />
                            </div>

                            <div className="relative">
                                <InstagramIcon className="absolute left-5 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-500" />
                                <input type="text" required value={instagram} onChange={e => setInstagram(e.target.value)} className="w-full p-5 pl-14 bg-dark border border-white/10 rounded-3xl text-white outline-none focus:ring-2 focus:ring-primary font-bold" placeholder="Usuário do Instagram" />
                            </div>

                            <button type="submit" disabled={isLoading} className="w-full py-5 bg-primary text-white font-black rounded-2xl shadow-xl shadow-primary/20 uppercase text-xs tracking-widest">
                                {isLoading ? 'PROCESSANDO...' : 'PROSSEGUIR PARA O PAGAMENTO'}
                            </button>
                        </form>
                    )}
                    ...
                    {step === 'payment' && pixData && (
                        <div className="space-y-8 text-center animate-fadeIn">
                            ...
                            <div className="bg-white p-4 rounded-3xl inline-block mx-auto shadow-2xl">
                                <img src={`data:image/jpeg;base64,${pixData.qr_code_base64}`} alt="QR Code Pix" className="w-56 h-56" />
                            </div>
                            ...
                            <div className="bg-dark/60 p-6 rounded-3xl border border-white/5 space-y-4">
                                ...
                                <div className="flex gap-2">
                                    ...
                                    <button type="button" onClick={() => { navigator.clipboard.writeText(pixData.qr_code); alert("Pix Copiado!"); }} className="p-3 bg-primary text-white rounded-xl">
                            ...
                            <div className="flex items-center justify-center gap-3 py-4 text-blue-400 font-bold animate-pulse">
                                ...
                                <span className="text-xs uppercase tracking-tighter text-center leading-tight">O sistema irá detectar seu pagamento<br/>automaticamente em poucos segundos</span>
                            </div>
                        </div>
                    )}
