...
const calculateAge = (dateString: string | undefined): string => {
    ...
    try {
        ...
        // Check if the date is valid. new Date('invalid') returns an Invalid Date object whose time is NaN.
                            ...
                            <table className="min-w-full divide-y divide-gray-700">
                                ...
                                <tbody className="bg-secondary divide-y divide-gray-700">
                                    {filteredPromoters.map((promoter) => (
                                        <tr key={promoter.id}>
                                            <td className="px-6 py-4 whitespace-nowrap"><div className="flex items-center"><div className="text-sm font-medium text-light">{promoter.name || 'N/A'}</div><span className="ml-2 px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-blue-900/50 text-blue-300">{promoter.state || 'N/A'}</span></div><div className="text-sm text-gray-400">{promoter.email || 'N/A'}</div><div className="text-sm text-gray-400">{calculateAge(promoter.dateOfBirth)}</div></td>
                                            <td className="px-6 py-4 whitespace-nowrap"><div className="flex flex-col space-y-2 text-sm"><a href={`https://wa.me/${(promoter.whatsapp || '').replace(/\D/g, '')}`} target="_blank" rel="noopener noreferrer" className="text-green-400 hover:underline inline-flex items-center"><WhatsAppIcon className="w-4 h-4 mr-2" /><span>{promoter.whatsapp || 'N/A'}</span></a><a href={formatSocialUrl(promoter.instagram, 'instagram')} target="_blank" rel="noopener noreferrer" className="text-pink-400 hover:underline inline-flex items-center"><InstagramIcon className="w-4 h-4 mr-2" /><span>Instagram</span></a>{promoter.tiktok && (<a href={formatSocialUrl(promoter.tiktok, 'tiktok')} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:underline inline-flex items-center"><TikTokIcon className="w-4 h-4 mr-2" /><span>TikTok</span></a>)}</div></td>
                        ...
                        <div className="md:hidden space-y-4">
                           {filteredPromoters.map((promoter) => (
                                <div key={promoter.id} className="bg-gray-900/50 p-4 rounded-lg shadow">
                                    <div className="flex justify-between items-start mb-3"><p className="font-bold text-lg text-light">{promoter.name || 'N/A'}</p><div className="flex flex-col items-end gap-2">{getStatusBadge(promoter.status)}<span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-blue-900/50 text-blue-300">{promoter.state || 'N/A'}</span></div></div><p className="text-sm text-gray-400">{promoter.email || 'N/A'}</p><p className="text-sm text-gray-400 mb-3">{calculateAge(promoter.dateOfBirth)}</p>
                                    ...
                                    <div className="border-t border-gray-700 pt-3 space-y-2 text-sm"><a href={`https://wa.me/${(promoter.whatsapp || '').replace(/\D/g, '')}`} target="_blank" rel="noopener noreferrer" className="text-green-400 hover:underline flex items-center"><WhatsAppIcon className="w-4 h-4 mr-2" /><span>{promoter.whatsapp || 'N/A'}</span></a><a href={formatSocialUrl(promoter.instagram, 'instagram')} target="_blank" rel="noopener noreferrer" className="text-pink-400 hover:underline flex items-center"><InstagramIcon className="w-4 h-4 mr-2" /><span>Instagram</span></a>{promoter.tiktok && (<a href={formatSocialUrl(promoter.tiktok, 'tiktok')} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:underline flex items-center"><TikTokIcon className="w-4 h-4 mr-2" /><span>TikTok</span></a>)}</div>
