...
import { Promoter, PromoterStatus, RejectionReason, AdminUserData, Organization } from '../types';
...
const PlanStatusBanner: React.FC<{ organization: Organization | null }> = ({ organization }) => {
    const [timeLeft, setTimeLeft] = useState('');

    useEffect(() => {
        if (!organization?.planExpiresAt) return;

        const interval = setInterval(() => {
            const expiresDate = (organization.planExpiresAt as any).toDate();
            const now = new Date();
    ...
    const isExpired = new Date() > (organization.planExpiresAt as any).toDate();
    const planStatus = organization.status === 'trial' ? 'Teste Gratuito' : 'Plano Ativo';
    
    const bgColor = isExpired ? 'bg-red-800/80 border-red-600' : 'bg-blue-800/80 border-blue-600';
    const textColor = isExpired ? 'text-red-200' : 'text-blue-200';

    return (
        <div className={`p-4 rounded-lg border-2 backdrop-blur-sm text-center mb-6 ${bgColor}`}>
            <p className={`font-bold text-lg ${isExpired ? 'text-white' : 'text-white'}`}>
                Status: {isExpired ? 'Expirado' : planStatus}
            </p>
            <p className={`font-mono text-xl my-2 ${textColor}`}>{timeLeft}</p>
            {organization.paymentLink && (
                <a 
                    href={organization.paymentLink}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="mt-2 inline-block px-6 py-2 bg-primary text-white font-semibold rounded-md hover:bg-primary-dark transition-colors"
                >
                    {isExpired ? 'Renovar Plano' : 'Pagar / Renovar'}
                </a>
            )}
        </div>
    );
...
const AdminPanel: React.FC<AdminPanelProps> = ({ adminData }) => {
    ...
    const isPlanExpired = useMemo(() => {
        if (!organization?.planExpiresAt) return true; // Default to expired if no date
        return new Date() > (organization.planExpiresAt as any).toDate();
    }, [organization]);
    ...
    const fetchOrgData = useCallback(async () => {
        if (adminData.organizationId) {
            try {
                ...
                setError("Não foi possível carregar os dados da sua organização.");
            }
        }
    ...
    }, [fetchPromoters, fetchReasons, canManage, adminData.organizationId, fetchOrgData, isSuperAdmin]);
    ...
    return (
        <div className="relative">
            {!isSuperAdmin && <PlanStatusBanner organization={organization} />}

            {isPlanExpired && !isSuperAdmin && (
                <div className="absolute inset-0 bg-dark bg-opacity-90 flex flex-col justify-center items-center z-20 rounded-lg">
                    <h2 className="text-2xl font-bold text-red-400">Plano Expirado</h2>
                    <p className="text-gray-300 mt-2">Sua assinatura terminou. Por favor, renove para reativar o painel.</p>
                </div>
            )}

            <div className={isPlanExpired && !isSuperAdmin ? 'blur-sm' : ''}>
