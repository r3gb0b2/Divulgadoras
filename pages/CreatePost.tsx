...
export const CreatePost: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setError(err.message || 'Falha ao enviar o formulário. Verifique os dados e tente novamente.');
        } finally {
        ...
        <div>
            <div className="flex justify-between items-center mb-6">
                 <div>
                    ...
                    <h1 className="text-3xl font-bold mt-1">
                        {location.search.includes('fromPost') ? 'Duplicar Publicação' : 'Criar Nova Publicação'}
                    </h1>
                </div>
            ...
            <form onSubmit={handleSubmit} className="bg-secondary shadow-lg rounded-lg p-6 space-y-6">
                ...
                <fieldset className="border border-gray-700 p-4 rounded-lg">
                    <legend className="text-lg font-semibold text-primary px-2">1. Direcionamento</legend>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-medium text-gray-300">Estado</label>
                            <select value={selectedState} onChange={e => setSelectedState(e.target.value)} required className="mt-1 w-full px-3 py-2 border border-gray-600 rounded-md bg-gray-700 text-gray-200">
                        ...
                        <div>
                            ...
                            <select value={selectedCampaign} onChange={e => setSelectedCampaign(e.target.value)} required disabled={!selectedState} className="mt-1 w-full px-3 py-2 border border-gray-600 rounded-md bg-gray-700 text-gray-200 disabled:bg-gray-800">
                ...
                {/* Step 2: Content */}
                <fieldset className="border border-gray-700 p-4 rounded-lg">
                    <legend className="text-lg font-semibold text-primary px-2">2. Conteúdo</legend>
                    <div className="space-y-4">
                        <div>
                            <label className="block text-sm font-medium text-gray-300">Nome do Evento (Opcional, para publicações específicas)</label>
                            <input type="text" value={eventName} onChange={e => setEventName(e.target.value)} placeholder="Ex: Festa Neon, Edição de Férias" className="mt-1 w-full px-3 py-2 border border-gray-600 rounded-md bg-gray-700 text-gray-200" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-300">Tipo de Post</label>
                            <div className="flex gap-4 mt-2">
                                <label><input type="radio" value="text" checked={postType === 'text'} onChange={() => setPostType('text')} /> Texto</label>
                                <label><input type="radio" value="image" checked={postType === 'image'} onChange={() => setPostType('image')} /> Imagem</label>
                                <label><input type="radio" value="video" checked={postType === 'video'} onChange={() => setPostType('video')} /> Vídeo</label>
                            </div>
                        </div>
                        {postType === 'text' && (
                            <textarea value={textContent} onChange={e => setTextContent(e.target.value)} rows={5} placeholder="Digite o texto do post aqui..." className="w-full px-3 py-2 border border-gray-600 rounded-md bg-gray-700 text-gray-200" />
                        )}
                        {postType === 'image' && (
                            <div>
                        ...
                        <div>
                            ...
                            <div className="flex gap-6 mt-2">
                                <label className="flex items-center space-x-2">
                                    <input type="checkbox" checked={postFormats.includes('story')} onChange={() => handleFormatChange('story')} className="h-4 w-4 text-primary bg-gray-700 border-gray-500 rounded focus:ring-primary" />
                                    <span>Story</span>
                                </label>
                                <label className="flex items-center space-x-2">
                                    <input type="checkbox" checked={postFormats.includes('reels')} onChange={() => handleFormatChange('reels')} className="h-4 w-4 text-primary bg-gray-700 border-gray-500 rounded focus:ring-primary" />
                                    <span>Reels</span>
                                </label>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea value={instructions} onChange={e => setInstructions(e.target.value)} rows={5} placeholder="Instruções para a postagem..." className="w-full px-3 py-2 border border-gray-600 rounded-md bg-gray-700 text-gray-200" required />
                             <div className="flex justify-between items-center mt-2">
                                <select onChange={(e) => setInstructions(e.target.value)} value={instructions} className="text-sm bg-gray-800 border border-gray-600 rounded px-2 py-1">
                                    <option value="">Usar modelo de instrução...</option>
                                    {instructionTemplates.map(t => <option key={t.id} value={t.text}>{t.text.substring(0, 50)}...</option>)}
                                </select>
                                <button type="button" onClick={() => setIsInstructionsModalOpen(true)} className="text-sm text-primary hover:underline">Gerenciar Modelos</button>
                            </div>
                        </div>

                        <div>
                            <label className="block text-sm font-medium text-gray-300">Link da Postagem (Opcional)</label>
                            <InputWithIcon Icon={LinkIcon} type="url" name="postLink" placeholder="Ex: link do post no instagram" value={postLink} onChange={e => setPostLink(e.target.value)} />
                        </div>
                    </div>
                </fieldset>
                ...
                {/* Step 3: Promoters */}
                <fieldset className="border border-gray-700 p-4 rounded-lg">
                    <legend className="text-lg font-semibold text-primary px-2">3. Divulgadoras</legend>
                    {promoters.length > 0 ? (
                        <>
                            <label className="flex items-center space-x-2 p-2 font-semibold">
                                <input type="checkbox" onChange={handleSelectAllPromoters} checked={selectedPromoters.size === promoters.length} className="h-4 w-4 text-primary bg-gray-700 border-gray-500 rounded" />
                                <span>Selecionar Todas ({selectedPromoters.size}/{promoters.length})</span>
                            </label>
                            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 max-h-60 overflow-y-auto p-2 border-t border-gray-700">
                                {promoters.map(p => (
                                    <label key={p.id} className="flex items-center space-x-2 p-1 rounded hover:bg-gray-700/50 cursor-pointer">
                                        ...
                                        <span className={`truncate ${p.hasJoinedGroup ? 'text-green-400 font-semibold' : ''}`} title={`${p.name} (${p.instagram})${p.hasJoinedGroup ? ' - no grupo' : ''}`}>{p.instagram || p.name}</span>
                                    </label>
                                ))}
                            </div>
                        </>
                ...
                <fieldset className="border border-gray-700 p-4 rounded-lg space-y-4">
                    <legend className="text-lg font-semibold text-primary px-2">4. Opções</legend>
                    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-400">Data Limite de Confirmação (opcional)</label>
                ...
                {/* Step 5: Scheduling */}
                <fieldset className="border border-gray-700 p-4 rounded-lg">
                    <legend className="text-lg font-semibold text-primary px-2">5. Envio</legend>
                    <div className="space-y-4">
                        <label className="flex items-center space-x-3 p-3 bg-gray-700/50 rounded-lg cursor-pointer">
                            <input type="checkbox" checked={isScheduling} onChange={(e) => setIsScheduling(e.target.checked)} className="h-4 w-4 text-primary bg-gray-800 border-gray-600 focus:ring-primary" />
                            <div>
                                <span className="font-medium text-gray-200">Agendar publicação</span>
                                <p className="text-sm text-gray-400">Selecione uma data e hora futuras para enviar esta publicação automaticamente.</p>
                            </div>
                        </label>
                        {isScheduling && (
                            <div className="pl-8 space-y-2">
                                <div className="flex gap-4">
                                    <input type="date" value={scheduleDate} onChange={e => setScheduleDate(e.target.value)} required={isScheduling} className="w-full px-3 py-2 border border-gray-600 rounded-md bg-gray-700 text-gray-200" style={{ colorScheme: 'dark' }} />
                                    <input type="time" value={scheduleTime} onChange={e => setScheduleTime(e.target.value)} required={isScheduling} className="w-full px-3 py-2 border border-gray-600 rounded-md bg-gray-700 text-gray-200" style={{ colorScheme: 'dark' }} />
                                </div>
                                {utcDisplayTime && <p className="text-xs text-yellow-400">{utcDisplayTime}</p>}
                            </div>
                        )}
                    </div>
                </fieldset>
                ...
                <div className="flex justify-end">
                    <button type="submit" disabled={isSubmitting || isLoading} className="px-8 py-3 bg-primary text-white font-semibold rounded-md hover:bg-primary-dark disabled:opacity-50">
                        {isSubmitting ? 'Enviando...' : (isScheduling ? 'Agendar Publicação' : 'Enviar Publicação')}
                    </button>
                </div>
            </form>
