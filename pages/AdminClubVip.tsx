...
const ManageCodesModal: React.FC<{ isOpen: boolean, onClose: () => void, event: VipEvent, onSaved: () => void }> = ({ isOpen, onClose, event, onSaved }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-sm z-[150] flex items-center justify-center p-6" onClick={onClose}>
            <div className="bg-secondary w-full max-w-lg p-8 rounded-[2.5rem] border border-white/10 shadow-2xl" onClick={e => e.stopPropagation()}>
                <h2 className="text-xl font-black text-white uppercase mb-4">Estoque: {event.name}</h2>
                <div className="mb-6 p-4 bg-dark/50 rounded-2xl border border-white/5 flex justify-between items-center">
                    <p className="text-[10px] font-black text-gray-500 uppercase">Disponível: <span className="text-primary">{currentStock ?? '...'}</span></p>
                </div>
                ...
                <div className="mt-8 flex gap-3">
                    <button onClick={onClose} className="flex-1 py-4 bg-gray-800 text-gray-400 font-black rounded-2xl uppercase text-xs">Sair</button>
        ...
        <div className="pb-40">
            <div className="flex justify-between items-center mb-8 px-4 md:px-0">
                <h1 className="text-3xl font-black text-white uppercase tracking-tighter flex items-center gap-3">
                    <TicketIcon className="w-8 h-8 text-primary" /> Gestão VIP
                </h1>
                ...
                {activeTab === 'members' && (
                    <div className="space-y-4">
                        <div className="relative">
                            ...
                            <input type="text" placeholder="BUSCAR..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full pl-11 pr-4 py-3 bg-dark border border-gray-700 rounded-2xl text-white text-[10px] font-black uppercase outline-none focus:border-primary" />
                        </div>
                                            ...
                                            <td className="px-6 py-5">
                                                <div className="flex flex-col items-center gap-2">
                                                    <div className="flex gap-3">
                                                        <div className={`p-1.5 rounded-lg border ${m.viewedAt ? 'bg-primary/20 border-primary text-primary' : 'bg-dark border-white/5 text-gray-700'}`} title={m.viewedAt ? `Visto ${formatDateSafe(m.viewedAt)}` : 'Não visto'}>
                                                            <EyeIcon className="w-4 h-4" />
                                                        </div>
                                                        <div className={`p-1.5 rounded-lg border ${m.downloadedAt ? 'bg-blue-600/20 border-blue-500 text-blue-400' : 'bg-dark border-white/5 text-gray-700'}`} title={m.downloadedAt ? `Baixado ${formatDateSafe(m.downloadedAt)}` : 'Não baixou'}>
                        ...
                        {vipEvents.map(ev => (
                            <div key={ev.id} className="bg-dark/40 p-6 rounded-[2rem] border border-white/5 group hover:border-primary transition-all flex flex-col">
                                ...
                                <div className="p-3 bg-white/5 rounded-2xl text-center mb-6">
                                    ...
                                    <p className="text-xl font-black text-white">{eventStats[ev.id]?.available ?? '...'}</p>
                                </div>
                                <div className="mt-auto flex flex-wrap gap-2">
                                    <button onClick={() => { setEventForCodes(ev); setIsCodesModalOpen(true); }} className="flex-grow py-3 bg-indigo-900/20 text-indigo-400 border border-indigo-800/30 rounded-xl text-[10px] font-black uppercase">ESTOQUE</button>
            ...
            {isModalOpen && (
                <div className="fixed inset-0 bg-black/90 backdrop-blur-sm z-[110] flex items-center justify-center p-6" onClick={() => setIsModalOpen(false)}>
                    <div className="bg-secondary w-full max-w-2xl p-8 rounded-[2.5rem] border border-white/10" onClick={e => e.stopPropagation()}>
                        <h2 className="text-2xl font-black text-white uppercase mb-6">Oferta VIP</h2>
                        <form onSubmit={handleSaveEvent} className="space-y-4">
                            ...
                            <button type="submit" className="w-full py-5 bg-primary text-white font-black rounded-2xl uppercase">SALVAR</button>
                        </form>
                    </div>
                </div>
            )}
